...
function _t(t, n, e, i) {
	if (t[2] && i) {
		...
		if (typeof c == 'object') {
			const s = [],
				u = Math.max(n.dirty.length, c.length);
			for (let o = 0; o < u; o += 1) s[o] = n.dirty[o] | c[o];
			return s;
		}
		return n.dirty | c;
	}
...
function U(t) {
	...
	for (let r = 0; r < n.length; r++) {
		const l = n[r].claim_order,
			a = (c > 0 && n[e[c]].claim_order <= l ? c + 1 : R(1, c, (y) => n[e[y]].claim_order, l)) - 1;
		i[r] = e[a] + 1;
		const f = a + 1;
	...
	for (let r = e[c] + 1; r != 0; r = i[r - 1]) {
		for (s.push(n[r - 1]); o >= r; o--) u.push(n[o]);
		o--;
	}
	for (; o >= 0; o--) u.push(n[o]);
	s.reverse(), u.sort((r, l) => r.claim_order - l.claim_order);
	for (let r = 0, l = 0; r < u.length; r++) {
		for (; l < s.length && u[r].claim_order >= s[l].claim_order; ) l++;
		const a = l < s.length ? s[l] : null;
		t.insertBefore(u[r], a);
	}
}
function W(t, n) {
	if (b) {
...
function mt(t, n, e) {
	b && !e ? W(t, n) : (n.parentNode !== t || n.nextSibling != e) && t.insertBefore(n, e || null);
}
function V(t) {
...
function B(t, n, e, i, c = !1) {
	...
	const s = (() => {
		for (let u = t.claim_info.last_index; u < t.length; u++) {
			...
			if (n(o)) {
				const r = e(o);
				return r === void 0 ? t.splice(u, 1) : (t[u] = r), c || (t.claim_info.last_index = u), o;
			}
		}
		for (let u = t.claim_info.last_index - 1; u >= 0; u--) {
			...
			if (n(o)) {
				const r = e(o);
				return (
					r === void 0 ? t.splice(u, 1) : (t[u] = r),
					c ? r === void 0 && t.claim_info.last_index-- : (t.claim_info.last_index = u),
					o
				);
			}
		}
	...
	return B(
		...
		(c) => {
			...
			for (let u = 0; u < c.attributes.length; u++) {
				const o = c.attributes[u];
				e[o.name] || s.push(o.name);
			}
			s.forEach((u) => c.removeAttribute(u));
		},
		() => i(n)
	);
}
...
function D() {
	if (!A) {
		...
		do {
			...
			for ($(null), p.length = 0; O.length; ) O.pop()();
...
function ut(t, n, e, i) {
	const { fragment: c, on_mount: s, on_destroy: u, after_update: o } = t.$$;
	c && c.m(n, e),
		i ||
			j(() => {
				const r = s.map(M).filter(H);
				u ? u.push(...r) : m(r), (t.$$.on_mount = []);
			}),
		o.forEach(j);
}
function ot(t, n) {
...
function lt(t, n) {
	t.$$.dirty[0] === -1 && (p.push(t), it(), t.$$.dirty.fill(0)),
		(t.$$.dirty[(n / 31) | 0] |= 1 << n % 31);
}
function Pt(t, n, e, i, c, s, u, o = [-1]) {
	const r = x;
	$(t);
	const l = (t.$$ = {
		fragment: null,
	...
	if (
		((l.ctx = e
			? e(t, n.props || {}, (f, y, ...C) => {
					const q = C.length ? C[0] : y;
					return (
						l.ctx &&
							c(l.ctx[f], (l.ctx[f] = q)) &&
							(!l.skip_bound && l.bound[f] && l.bound[f](q), a && lt(t, f)),
						y
					);
			  })
			: []),
		...
		n.intro && ct(t.$$.fragment), ut(t, n.target, n.anchor, n.customElement), Q(), D();
	}
	$(r);
}
class Dt {
...
function Ft(t, n = d) {
	...
	return { set: c, update: s, subscribe: u };
}
export {
	jt as A,
	F as B,
